...
// Libraries/MLXLLM/Models/GPTOSS.swift — copied so the Lens text encoder can capture
// per-layer hidden states (layers are fileprivate upstream) and inject the GPT-OSS
// YaRN rope when the checkpoint config omits rope_scaling (the no-config fallback
// upstream constructs YarnRoPE(scalingFactor: 1.0) == plain rope — the F9 trap).
...
// MARK: - Lens addition (NOT upstream): multi-layer hidden-state capture.
// Mirrors lens_mlx/model/text_encoder.py: full + sliding causal masks selected per
// layer_type (T8), capture post-layer at the selected indices (T0 — 0-indexed),
// early-exit after the last selected layer; final norm + lm_head are skipped.
extension GPTOSSModelInner {
    public func captureHiddenStates(
        _ inputs: MLXArray, selectedLayers: [Int]
    ) -> [MLXArray] {
        precondition(!selectedLayers.isEmpty)
        precondition(Set(selectedLayers).count == selectedLayers.count)
        precondition(selectedLayers.min()! >= 0 && selectedLayers.max()! < layers.count)
        let maxLayer = selectedLayers.max()!

        var x = embedTokens(inputs)
        let seqLen = x.dim(1)
        let fullMask = makeAttentionMask(n: seqLen, cache: nil, windowSize: nil)
        let swaMask = makeAttentionMask(n: seqLen, cache: nil, windowSize: windowSize)

        var captured: [Int: MLXArray] = [:]
        for (i, layer) in layers.enumerated() {
            let mask = layerTypes[i] == "full_attention" ? fullMask : swaMask
            x = layer(x, mask: mask, cache: nil)
            if selectedLayers.contains(i) {
                captured[i] = x
            }
            if i == maxLayer { break }
        }
        let result = selectedLayers.map { captured[$0]! }
        eval(result)
        return result
    }
}
